...
public class MetricsServiceImpl extends DefaultComponent implements MetricsService {
    ...
    protected void updateInstrumentation(List<MetricsDescriptor.InstrumentDescriptor> instruments, boolean activate) {
        for (String instrument : instruments.stream()
                                            .filter(MetricsDescriptor.InstrumentDescriptor::isEnabled)
                                            .map(MetricsDescriptor.InstrumentDescriptor::getId)
                                            .collect(Collectors.toList())) {
            switch (instrument) {
            case "log4j":
